 ...
 *
 *********************************************************************
 * FileName:        adc_lcd_display_B_mx7.c
 * Dependencies:    plib.h
 *
 ...
 *      serial LCD display via the SPI interface on the Chipkit MX4.
 *      In addition, this version also converts the integer value
 *      to a floating point value and also transmits it to the
 *      LCD Display.      
 *
 ********************************************************************/
...
int main(void)
{
    ...
    char adc_str[16];		// char string for viewing ADC value
    float bit_value;		// weight of one ADC bit
    float analog_value;		// floating point representation of
                            // the integer value.
				
    bit_value = (VMAX - VMIN)/ADC_COUNT;	// weight of one bit
...
//	sprintf(adc_str,"%d",a );	// convert int to ASCII string	
	analog_value = (float)a * bit_value;
	sprintf(adc_str,"%6.3f V",analog_value );	// convert int to ASCII string
	putsSPI1(8,adc_str);	// write out string
	DelayMs(1000);		// wait for display to reset
...
void DelayInit()	
{
	...
	/* Configure Timer 2. This sets it up to count a 10 MHz clock 	*/
