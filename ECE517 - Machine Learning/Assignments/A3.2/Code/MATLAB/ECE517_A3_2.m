...
plot(X(1:end/2,1),X(1:end/2,2),'+', LineWidth=1.5)
grid on
grid minor
hold all
plot(X(end/2+1:end,1),X(end/2+1:end,2),'o', LineWidth=1.5)
title("\sigma = " + sigma);
hold off
%% 
...
plot(plot_x, d_plus, 'r--', LineWidth=1.5);
plot(plot_x, d_minus, 'b--', LineWidth=1.5);
...
error_train_indexed(k) = sum(error_train)/length(error_train);  % Empirical Risk
error_indexed(k) = sum(error)/length(error);                    % Actual Risk
diff_error(k) = error_indexed(k) - error_train_indexed(k);      % Structural Risk
...
leg1 = legend([p1,p2, p3],{'Empirical Risk','Actual Risk','Structural Risk'},'Location','best');
grid minor
