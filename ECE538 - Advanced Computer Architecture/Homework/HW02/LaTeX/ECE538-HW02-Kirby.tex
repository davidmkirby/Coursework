...
You are designing a System On a Chip (SoC) with your 5-stage pipelined RISC processor. The minimum system requirements are 512KB of program ROM and 16KB of RAM. For a given technology node, the critical path of your processor (ignoring memory access) is 1 ns. The access time for a 512KB ROM in the same technology is 2.0 ns, while the access time for a 16KB RAM is 0.8 ns.
    \begin{parts}
        ...
        \begin{tcolorbox}[colback=CrispBlue!5!white,colframe=CrispBlue!75!black,title=]
            \begin{align*}
                \text{Miss penalty} &= 1^{\text{st}}\text{ cycle to detect miss} + \text{\# of ROM to fill cache} \times \text{Time to determine \# of cycles to read the ROM}\\
                &= 1\text{ cc} + \text{ROM} \times 2.1 \text{ ns}\\
                \frac{\text{Memory stall cycles}}{\text{Instruction}} &= \text{Instruction count} \times \text{\# of misses} \times \text{Miss penalty}\\
                &= \\
                &= 
            \end{align*}
        \end{tcolorbox}
        ...
        \begin{tcolorbox}[colback=CrispBlue!5!white,colframe=CrispBlue!75!black,title=]
            \begin{align*}
                \text{CPI}_{\text{initial}} &= \frac{4.8\text{ million clock cycles}}{4 \text{ million instructions}}\\
                &= 1.2\ \frac{\text{clock cycles}}{\text{instruction}}\\[1em]
                \frac{\text{Stalls}}{\text{Instruction}} &= 1 \text{ cycle}
            \end{align*}
        \end{tcolorbox}
        \vspace{\stretch{1}}
    \end{parts}
...
Assume that both \texttt{X} and \texttt{Y} are stored in the row major order and each element in the matrices is a double word (64-bit) integer.
    \begin{parts}
        ...
        \begin{tcolorbox}[colback=CrispBlue!5!white,colframe=CrispBlue!75!black,title=]
            The main purpose of loop-blocking is to eliminate as many cache misses as possible by breaking up the memory into smaller chunks. In this case I chose to break it up into 8-bit chunks, and then transposing from there.
        \end{tcolorbox}
        \vspace{\stretch{1}}
